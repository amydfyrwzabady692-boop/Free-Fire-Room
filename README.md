...
در پنل برگزارکننده، صف اسکرین‌ها هم **تک‌تک** («تأیید / رد» زیر هر عکس) و هم
**یک‌جا** («تأیید همه» / «رد همه») قابل بررسی است. تأیید گروهی همهٔ ثبت‌نام‌ها را قطعی
می‌کند و ROOM ID / PASS را با **یک** ارسال برای کل دسته می‌فرستد، نه یکی به‌ازای هر نفر.
