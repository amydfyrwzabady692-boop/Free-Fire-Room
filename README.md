...
## اطلاع‌رسانی قبل از هر کاستوم

ربات هر کاستوم جایزه‌دار را **یک ساعت قبل** و **ده دقیقه قبل** از شروع اعلام می‌کند:

- کسی که ثبت‌نام کرده، یادآوری می‌گیرد («فقط در کانال‌ها بمانید»).
- بقیهٔ کاربران ربات دعوت می‌گیرند، با جایزه و ساعت و یک دکمهٔ ورود مستقیم.

هر ارسال با یک قفل Redis روی «کاستوم + فاصله» محافظت می‌شود تا دوبار نرود، با همان
سرعت `TELEGRAM_OUTBOUND_PER_SECOND` فرستاده می‌شود و به `TelegramRetryAfter` احترام
می‌گذارد. با `EVENT_REMINDER_BROADCAST=false` فقط ثبت‌نام‌کرده‌ها خبر می‌گیرند.
